...
namespace Ffmpeg
{
    public class FfmpegConverter
    {
        ...
        //private string videosize = "800*500"; //视频大小
        ...
        /// <summary>
        /// 返回缩略图片路径格式为: FileName+_thumb.jpg
        /// </summary>
        public string ConvertImage(string sourceFile, string imgDirectory, string imgName)
        {
            string ffmpeg = System.Web.HttpContext.Current.Server.MapPath(ffmpegpath);
            ...
            ////截大图
            string imgpath = imgDirectory + imgName + "_thumb.jpg";// FileHelper.GetUniqueFileName(".jpg");

            ImgstartInfo.WindowStyle = System.Diagnostics.ProcessWindowStyle.Hidden;
            ImgstartInfo.UseShellExecute = false;
            ImgstartInfo.CreateNoWindow = true;
            ImgstartInfo.RedirectStandardError = true;
            ...
            VideoLength = GetVideoDuration(ffmpeg, sourceFile);  //提取文件长度
        ...
        /// ImgSize格式如:325*200
        /// </summary>
        public string ConvertImage(string sourceFile, string imgDirectory, string imgName, string ImgSize)
        {
            string ffmpeg = System.Web.HttpContext.Current.Server.MapPath(ffmpegpath);
            ...
            {
                string part = @"\d{1,3}\*\d{1,3}";
                if (Regex.IsMatch(ImgSize, part))
                {
                    imgsize = ImgSize;
                }
                else
                {
                    throw new Exception("输入的格式不正确!例:'325*200'");
                }
            }
        ...
        public string ConvertImage(string sourceFile, string imgpath, string imgsize, System.Diagnostics.ProcessStartInfo ImgstartInfo)
        {
            ImgstartInfo.WindowStyle = System.Diagnostics.ProcessWindowStyle.Hidden;
            /*参数设置
             * -y（覆盖输出文件，即如果生成的文件（flv_img）已经存在的话，不经提示就覆盖掉了）
