<%@ Page Language="C#" MasterPageFile="~/Masters/Ext/Site.Master" AutoEventWireup="true"
    CodeBehind="Home3.aspx.cs" Inherits="Aim.Examining.Web.Home3" Title="首页" %>
    ...
    <script type="text/javascript">
        ...
        function setPgUI() {
            ...
            */
            ...
            var recs = (tsk);
            recs.push({ title: '消息提醒', count: AimState["MsgCount"] || "0", url: '/Modules/Office/SysMessageTab.aspx', iconUrl: '/images/Iphone/1.ico' });
            recs.push({ title: '新闻通知', count: AimState["NewsCount"] || "0", url: 'Modules/PubNews/NewsList.aspx?op=r', iconUrl: '/images/Iphone/2.ico' });
            pal = buildInfoView({ title: '办公信息汇总', url: '/workflow/TaskTab.aspx?TypeId={id}', mappings: { 'id': 'Id' }, records: recs });
            infoPanels.push(pal);
            //pal = buildInfoView({ title: '消息提醒', url: '', records: [{ title: '消息提醒', count: AimState["MsgCount"] + "条" || "0", url: '/Modules/Office/SysMessageTab.aspx'}] });
            //infoPanels.push(pal);
            ...
            //infoPanels.push(pal);
            var pal = buildInfoView({ ICO: true, title: '我的快捷<span style=width:95px></span><span onclick="showshort()" style=cursor:hand;color:blue;>设置</span>', url: '{url}', mappings: { 'url': 'ModuleUrl', 'fileid': 'fileid' }, records: ($.getJsonObj(AimState["ShortCut"]).length > 0 ? $.getJsonObj(AimState["ShortCut"]) : [{}]) });
            infoPanels.push(pal);
            //项目相关
            //recordsArr = [];
            ...
            linkHTML += '<b style="color:black;">我的链接<span style=width:95px></span><span onclick="weblinks()" style=cursor:hand;color:blue;>设置</span></b>';
            linkHTML += '<hr />';
            linkHTML += '<span style="width:80%;"><select style="width:100%" onchange=if(this.value)window.open(this.value,"_blank")>';
            linkHTML += '<option>请选择...</option>';
