...
namespace Aim.Portal.Services
{
    ...
    public class UserSessionService : IUserSessionService
    {
            ...
            try
            {
                ...
                else if (label == "getsystemdata")
                {
                    using (new SessionScope())
                    {
                        ...
                        else if (opMsg.Operation == "getonlineusers")
                        {
                            dataObj = Server.Sessions.Select(en => en.Value.User).ToArray();
                        }
                    }
                }
                else
        ...
        public string GetUserDataJson(string msg)
        {
            try
            {
                OpMessage opMsg = new OpMessage(msg);
                ...
                return ExecuteServiceByMsgString(opMsg);
            }
            catch (Exception ex)
            {
                return "";
        ...
        public string GetSystemDataJson(string msg)
        {
            try
            {
                OpMessage opMsg = new OpMessage(msg);
        ...
        private string ExecuteServiceByMsgString(OpMessage opMsg)
        {
            try
            {
                string data = null;
                ...
                else if (label == "getsystemdata")
                {
                    using (new SessionScope())
                    {
                        ...
                        else if (opMsg.Operation == "getallusergroup")
                        {
                            dataObj = DataHelper.QueryDictList("select * from SysUserGroup");
                        }
                        else if (opMsg.Operation == "getalluserrole")
                        {
                            dataObj = DataHelper.QueryDictList("select * from SysUserRole");
                        }
                    }
                }
                else
                ...
                if (dataObj != null)
                {
                    data = JsonHelper.GetJsonString(dataObj);
                }
            ...
            {
                if (opMsg.Operation == "getsessionbyloginname")
                {
                    return Server.GetSessionByLoginName(opMsg.SessionID).SessionID;
                }
                return null;
            }
        }
