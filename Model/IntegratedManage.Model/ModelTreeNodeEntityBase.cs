...
{
    public abstract class ModelTreeNodeEntityBase<T> : IMModelBase<T>, ITreeNodeEntity<T> where T : ModelTreeNodeEntityBase<T>, new()
    {
        #region 成员变量
        ...
        protected virtual void ChangeParent(T target, int position)
        {
            ...
            ChangeParent(conn, target, position);
        }
        ...
        {
            ISession session = holder.CreateSession(this.GetType());
        ...
        /// </summary>
        /// <param name="position"></param>
        public virtual void ChangePosition(IDbConnection conn, int position)
        {
            ISession session = DataHelper.OpenHqlSession(conn);
        ...
        {
            //AimIdentifierGenerator idGenerator = new AimIdentifierGenerator();
